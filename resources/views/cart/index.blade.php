            ...
            <div class="flex-1 space-y-6">
                @foreach($items as $item)
                    <div class="flex items-center gap-6 bg-white p-6 rounded-2xl shadow-sm border border-gray-100/50 transition hover:shadow-md">
                        <div class="w-24 h-24 shrink-0 bg-gray-50 rounded-xl flex items-center justify-center overflow-hidden">
                            <x-product-image
                                :url="$item['product']->image_url"
                                :alt="$item['product']->name"
                                class="{{ $item['product']->image_url ? 'h-full w-full object-cover' : 'w-10 h-10 text-gray-300' }}"
                            />
                        </div>
