...
<div class="max-w-4xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm mt-4 overflow-hidden">
        <div class="p-6">
                <p class="text-sm text-indigo-500 font-medium">{{ $product->category->name ?? 'Uncategorized' }}</p>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-1">{{ $product->name }}</h1>
                <p class="text-gray-600 dark:text-gray-300 mt-3">{{ $product->description }}</p>
                <p class="text-2xl text-indigo-600 dark:text-indigo-400 font-bold mt-4">${{ number_format($product->price, 2) }}</p>
                ...
                @auth
                    <form action="{{ route('cart.add', $product) }}" method="POST" class="mt-6 flex items-center gap-3">
                        @csrf
                        <input type="number" name="quantity" value="1" min="1" class="w-20 border border-gray-300 dark:border-gray-600 rounded px-3 py-2 dark:bg-gray-700 dark:text-gray-200">
                        <button type="submit" class="bg-indigo-600 text-white px-6 py-2 rounded hover:bg-indigo-700 transition font-medium">Add to Cart</button>
                    </form>
                @else
